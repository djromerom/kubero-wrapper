...
export default function StatusBadge({ status }: Props) {
  return (
    <span className={`px-2 py-1 rounded text-xs font-medium ${colors[status] || 'bg-atlas-mist text-atlas-muted'}`}>
      {({ pending: getDemoUser() ? 'Validación pendiente' : 'Despliegue pendiente', building: getDemoUser() ? 'CI en ejecución' : 'Desplegando', ci_pending: 'CI pendiente', corrections: 'Correcciones pendientes', retained: 'Retenido', recovery_pending: 'Recuperación pendiente', ci_failed: 'CI fallido', approved: 'Aprobado', passed: 'Superado', not_started: 'No iniciado', not_deployed: 'No desplegado', running: 'Desplegado', failed: 'Despliegue fallido', deleted: 'Eliminado' } as Record<string, string>)[status] || status}
    </span>
  );
}
